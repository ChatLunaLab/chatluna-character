...
export function apply(ctx: Context, config: Config) {
    ...
    ctx.command('character.event', '角色事件循环')
        .action(async ({ session }) => {
            return '角色事件循环系统，用于管理角色的日常事件循环。\n' +
    ...
    ctx.command('character.event.activate <presetKey:string>', '激活预设的事件循环')
    ...
    ctx.command('character.event.deactivate <presetKey:string>', '停用预设的事件循环')
        .action(async ({ session }, presetKey) => {
            if (!presetKey) {
                return '请指定要停用的预设关键词';
            }
    ...
    ctx.command('character.event.list', '列出所有已激活的预设事件循环')
        .action(async ({ session }) => {
            const activePresets = ctx.chatluna_character_event_loop.getActivePresets();
    ...
    ctx.command('character.event.current <presetKey:string>', '显示预设的当前事件')
        .action(async ({ session }, presetKey) => {
            if (!presetKey) {
                return '请指定预设关键词';
            }
            ...
            try {
                ...
                const currentEvent = await ctx.chatluna_character_event_loop.getCurrentEvent(presetKey);
                if (!currentEvent) {
                    return `预设 "${presetKey}" 当前没有事件`;
                }

                const description = await ctx.chatluna_character_event_loop.getCurrentEventDescription(presetKey);

                const startTime = currentEvent.timeStart.toLocaleTimeString('zh-CN', {
                    hour: '2-digit',
                    minute: '2-digit'
                });
                ...
                let response = `预设 "${presetKey}" 的当前事件:\n` +
    ...
    ctx.command('character.event.all <presetKey:string>', '显示预设的所有事件')
        .action(async ({ session }, presetKey) => {
            if (!presetKey) {
                return '请指定预设关键词';
            }
            ...
            try {
                ...
                const events = await ctx.chatluna_character_event_loop.getEvents(presetKey);
                if (!events || events.length === 0) {
                    return `预设 "${presetKey}" 没有事件`;
                }

                const eventsList = events.map(event => {
                    const startTime = event.timeStart.toLocaleTimeString('zh-CN', {
                        hour: '2-digit',
                        minute: '2-digit'
                    });
