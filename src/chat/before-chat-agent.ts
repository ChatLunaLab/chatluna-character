...
import { BEFORE_CHAT_PROMPT } from './prompt'
import { AgentFinish } from '@langchain/core/agents'
import { PresetTemplate } from '../types'

export interface BeforeChatAgentInput extends BaseAgentInput {
    characterPrompt: PresetTemplate
}

export class BeforeChatAgent extends BaseAgent {
    characterPrompt: PresetTemplate

    constructor(input: BeforeChatAgentInput) {
        super(input)
    ...
    get prompt(): CharacterPrompt {
        if (!this._prompt) {
            this._prompt = new CharacterPrompt({
                ...
                systemPrompt:
                    SystemMessagePromptTemplate.fromTemplate(BEFORE_CHAT_PROMPT)
            })
        }
        return this._prompt
    }
