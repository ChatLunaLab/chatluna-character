...
export function apply(ctx: Context, config: Config) {
    ...
    ctx.on('ready', async () => {
        ...
        const name = driver.constructor.name.toLowerCase()

        if (!name.includes('sqlite') && !name.includes('mysql')) {
            return
        }

        if (name.includes('sqlite')) {
            const cols = driver._all(
                'PRAGMA table_info(`chathub_character_variable`)'
            )
            ...
            if (
                !cols.some((col) => col.name === 'groupId') &&
                !cols.some((col) => col.name === 'statusMessageTimestamp') &&
                !cols.some((col) => col.name === 'statusMessageContent')
            ) {
                return
            }

            const rows = driver._all(
                "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'chathub_character_wake_up_reply'"
            )

            await db.drop('chathub_character_variable')
            if (rows.length) {
                await db.drop('chathub_character_wake_up_reply')
            }
        ...
        const cols = await driver._all(
            "SELECT COLUMN_NAME AS name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'chathub_character_variable'"
        )
        ...
        const rows = await driver._all(
            "SELECT TABLE_NAME AS name FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'chathub_character_wake_up_reply'"
        )
